...
export interface BuildTierStatusTxParams {
  tx: Transaction;
  packageId: string;
  platformId: string;
  platformInitVersion: number;
...
export function buildDeactivateTierTx(params: BuildTierStatusTxParams) {
...
export function buildActivateTierTx(params: BuildTierStatusTxParams) {
  const { tx, packageId, platformId, platformInitVersion, tierIndex } = params;
  tx.moveCall({
    target: `${packageId}::platform::activate_tier_by_index`,
    arguments: [
      tx.sharedObjectRef({
        objectId: platformId,
        initialSharedVersion: platformInitVersion,
