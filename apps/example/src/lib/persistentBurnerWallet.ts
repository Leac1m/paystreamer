...
function loadOrCreateKeypair(): Ed25519Keypair {
	if (typeof window === 'undefined') return Ed25519Keypair.generate(); // SSR fallback
...
export function createPersistentBurnerWalletInitializer(): WalletInitializer {
	return {
		...
		initialize({ networks, getClient }) {
			try {
				const wallet = new PersistentBurnerWallet({ clients: networks.map((network) => getClient(network)) as any });
