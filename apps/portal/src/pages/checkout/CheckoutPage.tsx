...
import { NETWORK_CONFIGS, SupportedNetwork, CLOCK_OBJECT_ID, NETWORK } from "../../constants";
...
function CheckoutExperience() {
  ...
  const showSpinner = platformLoading && !platform;
  ...
  const handleSubscribeClick = (tierIndex: number, amount: bigint, frequency: bigint) => {
    if (!account) {
      modalRef.current?.show();
      return;
    }
    setSelectedTierParams({ index: tierIndex, amount, frequency });
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
                ...
                {activeTiers.map((tier, index) => (
                  <div key={index} className="relative overflow-hidden bg-[#151520] border border-white/10 rounded-xl p-6">
                    ...
                    {!account ? (
                      ...
                      <button
                        onClick={() => handleSubscribeClick(index, BigInt(tier.amount), BigInt(tier.frequency))}
                        className="w-full bg-[#6c63ff] hover:bg-[#5a52d5] transition-colors text-white py-2 rounded-lg"
                      >
                        Subscribe
                      </button>
      ...
      {selectedTierParams && (
        <SetupSubscriptionModal
          ...
          tierFrequencyMs={selectedTierParams.frequency}
          onSuccess={() => {
            setIsSetupModalOpen(false);
