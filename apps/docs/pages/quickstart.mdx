...
```bash
npm install @paystreamer/sdk @mysten/dapp-kit-react@2.1.10 @mysten/sui@2.23.1 @tanstack/react-query
```

<Callout type="info">
  **Compatibility matrix**: `@mysten/sui@2.23.1`, `@mysten/dapp-kit-react@2.1.10`, `@mysten/dapp-kit-core@1.6.8`, `@tanstack/react-query@^5.90.16`, React 18 or 19, Node 20+. Sui CLI `1.75.x` for local development. See the [SDK README](https://github.com/paystreamer) for the current matrix.
</Callout>
...
<Callout type="warning">
  The example below subscribes to an existing `platformId` — it doesn't create one. If you don't have a platform registered yet, do that first: [Register Your Platform & Create Tiers](/platform-setup).
</Callout>
