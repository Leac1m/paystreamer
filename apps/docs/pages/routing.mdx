...
This is exactly what `packages/scheduler-core/src/routedPayment.ts` does. The full opt-in decision lives in `packages/scheduler-core/src/routing.ts`'s `classifyPayment`:
