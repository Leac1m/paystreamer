...
#[test_only]
module paystreamer_v2::scheduler_tests {
    ...
    fun test_pause_unpause_round_trip() {
        ...
        // Limiter is not initialized (deferred-init pattern).
        assert!(!scheduler::is_initialized(&scheduler), 3);
    ...
    // with `ESchedulerPaused` (0x0A002) before touching the global
    // limiter or the account. State assertions: balance unchanged,
    // nonce unchanged, last_processed_at unchanged.
    ...
    // === test_process_due_payment_global_rate_limited ===
    //
    // The global bucket starts at 10_000 tokens. After 10_000
    // successful `process_due_payment` calls, the 10_001st call must
    // abort with `EGlobalRateLimited` (0x0A001). The test does not
    // require 10_000 full billing cycles — the global limiter is
    // checked BEFORE the schedule / balance checks inside the
    // scheduler, so each iteration only needs the limiter consume
    // to succeed. We drive the bucket to zero by repeatedly calling
    // `rate_limiter::try_consume` directly on a scheduler-borrowed
    // limiter (the access pattern matches what `process_due_payment`
    // does internally), then assert the 10_001st consume aborts.
    //
    // This pins the bucket capacity invariant: the global circuit
    // breaker is 10k tokens, no more, no less. If a future change
    // bumps the cap, this test is the canary.

    #[test]
    #[expected_failure(abort_code = 0x0A001)]
    fun test_process_due_payment_global_rate_limited() {
        let owner = @0xA;
        let mut sc = ts::begin(owner);
        let clock = fresh_clock(&mut sc);
        ...
        // Drain the global bucket to zero. 10_000 successful
        // `try_consume(1, ...)` calls bring the bucket from 10_000
        // down to 0. The 10_001st must abort. The
        // `global_limiter_mut_for_testing` accessor lazily
        // initializes the limiter on first call (mirroring the
        // production deferred-init pattern).
        let mut i: u64 = 0;
        while (i < 10_000) {
            let ok = rate_limiter::try_consume(
                scheduler::global_limiter_mut_for_testing(&mut scheduler, &clock),
                1,
                &clock,
            );
            assert!(ok, i); // sanity: each consume must succeed within capacity
            i = i + 1;
        };

        // 10_001st consume: bucket is empty, must abort with
        // 0x0A001 (EGlobalRateLimited). To pin the user-visible
        // abort path through `process_due_payment`, we now share
        // the drained scheduler, set up a real account+platform
        // pair, and route the 10_001st call through
        // `process_due_payment`. The scheduler checks the global
        // limiter FIRST (before the schedule / balance / policy
        // checks), so the call aborts before any state mutation.
        let r = registry_with_test_usdc(&mut sc);
        let (account_id, platform_id) = setup_account_with_subscription(
            &r,
            &clock,
        ...
        // Share the scheduler (drained bucket).
        let scheduler_id = object::id(&scheduler);
        scheduler::share_for_testing(scheduler);
        ...
        // Must abort with 0x0A001 (EGlobalRateLimited). The
        // scheduler checks the global limiter FIRST, so the call
    ...
    // paused, bucket full) submits a due payment. We verify:
    ...
    //
    // This is the canary for the "permissionless but rate-limited"
    // property: a gas-paying third party can drive the full
    ...
    fun test_process_due_payment_succeeds_when_not_paused() {
        ...
        assert!(!scheduler::is_paused(&scheduler), 7);
        // Limiter is now initialized (deferred init triggered on
        // the first call). One token was consumed (10_000 -> 9_999).
        assert!(scheduler::is_initialized(&scheduler), 8);
        assert!(rate_limiter::available(
            scheduler::global_limiter(&scheduler),
            &clock,
        ) == 9_999, 9);
    ...
    // === Test-only helper: mutable handle to the global limiter ===
    //
    // `rate_limiter::try_consume` requires `&mut RateLimiter`. The
    // public `global_limiter` accessor returns `&RateLimiter`; we
    // call the `#[test_only]` `scheduler::global_limiter_mut_for_testing`
    // helper from this test module to drain the bucket in
    // `test_process_due_payment_global_rate_limited`. The `&mut`
    // accessor is `#[test_only]` so production code can never reach
    // in and consume the bucket outside the scheduler's own gated
    // path. (No wrapper needed: the test calls
    // `scheduler::global_limiter_mut_for_testing` directly.)
}
